...
# ╔═╡ 05fd19bc-a23e-4bd1-ab9a-5aedc5a836ae
using CairoMakie

# ╔═╡ d0a2ece8-269e-11f0-1b02-6b94ad8c7cce
using SummationByPartsOperators

# ╔═╡ 9660cc59-07f4-4d59-9fe6-4bc4e5193e3a
using OrdinaryDiffEqSDIRK

# ╔═╡ fcc24124-d92d-4519-a532-2c5f493249d4
using BenchmarkTools

# ╔═╡ fec529da-915f-498b-865e-13a2121ee991
...
# ╔═╡ d47f3770-8d5f-4973-9bf7-f23c9f8e0c93
# inital conditions
f(x) = sin(x * π)

# ╔═╡ 889b782f-9a35-4b03-a486-4e7dec6c2bf7
begin
    xmin = 0.0
    xmax = 1.0
...
# ╔═╡ 4b1c8aea-8813-4403-9d38-04b045426d51
begin
    D_local = legendre_derivative_operator(xmin = -1.0, xmax = 1.0, N = polydeg + 1)
    mesh = UniformPeriodicMesh1D(; xmin, xmax, Nx = elements)
end

# ╔═╡ 6b7e51ee-f3cb-4b8f-8b01-47d83eafb7ec
begin
    D1m = couple_discontinuously(D_local, mesh, Val(:minus))
    D1p = couple_discontinuously(D_local, mesh, Val(:plus))
end

# ╔═╡ 0935f639-7239-4b50-8f76-ba8c8c73fed5
x = grid(D1m);

# ╔═╡ 66d9c4b1-3427-4922-8220-bcaae421da16
u₀ = f.(x);

# ╔═╡ 944f337c-543b-4033-9d1c-437211d3f6b2
lines(x, u₀)

# ╔═╡ a62da805-6251-4ae3-bc4c-39bc0bf19d6e
tspan = (0.0, 50.0)

# ╔═╡ 9874c9ba-4820-4d8c-8cc9-6b8b0dd998d2
prob = ODEProblem(heat_1D!, u₀, tspan, (D1m, D1p))

# ╔═╡ 662d72bd-4a27-4988-b898-00cab9d5e2f9
@time solve(prob, ImplicitEuler());

# ╔═╡ f0cd6938-b637-4deb-9c34-29f73fb4721c
Δt = 0.01

# ╔═╡ 13da60a5-0657-40a4-a657-515bc4584199
sol_M = solve(prob, ImplicitMidpoint(), dt = Δt)

# ╔═╡ 469d51c1-d7c8-4278-960f-c5eee80bd2fc
...
# ╔═╡ 6c3efaa7-323a-46c5-a921-95eb065039c8
@time solve(prob, ImplicitMidpoint(), dt = Δt);

# ╔═╡ 960b8fa5-2450-438e-a892-97846ea52143
plot_timesteps(x, sol_M.u, sol_M.t, [1, 2, 3, 4, 5, 10, length(sol_M.t)], title = "DiffEq -- ImplicitMidpoint Δt=$(Δt)")
...
manifest_format = "2.0"
project_hash = "1745fbde7b33b273c28d66a01481322afb870c61"
...
git-tree-sha1 = "8e473101c9dc0ee3825908b402c79fef33079fcd"
...
git-tree-sha1 = "b68c69d37ab318c0a506f43fd3f31de4a9ccdca2"
